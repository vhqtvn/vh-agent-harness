...
- private operator reminders -> `.local/{{COORDINATOR_DIR}}/`
...
`.local/{{COORDINATOR_DIR}}/tasks/` (via `/write-task`) with Notes-prefix
...
5. For `p2` items, do NOT add a backlog row — capture to
   `.local/{{COORDINATOR_DIR}}/tasks/` for later curation.
...
- `p2`: optional cleanup or follow-up task discovered during closeout -> capture to `.local/{{COORDINATOR_DIR}}/tasks/`, promote only on trigger + DoR
