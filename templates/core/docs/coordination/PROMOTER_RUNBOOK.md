...
during a cycle lives in `.local/{{COORDINATOR_DIR}}/tasks/`; the backlog is the
promoted, durable view.

## Procedure

1. **Read the live transport.** Open `.local/{{COORDINATOR_DIR}}/tasks/` and
...
- **Workers** reading the backlog mid-cycle may see stale `in_progress` /
  assignment rows. The `.local/{{COORDINATOR_DIR}}/tasks/` cards are the live
  view during a cycle.
...
1. Read all task cards in `.local/{{COORDINATOR_DIR}}/tasks/` since the last
   promotion.
