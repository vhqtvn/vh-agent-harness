...
| `.local/{{COORDINATOR_DIR}}/tasks/` | Conditional candidate holding area | Gitignored **transport, not truth** — unpromoted candidates may be lost (intentionally) | Curated into `backlog.md` by the promoter only after trigger + Definition of Ready | Manual / lossy by design |
...
| Runtime messaging plane | Optional fan-out/fan-in transport for long async work | `.local/{{COORDINATOR_DIR}}/` runtime files, local dashboards, report queues |
...
4. Keep `.local/{{COORDINATOR_DIR}}/` local-only and operator-specific.
