...
- `.local/AGENTS.md` when working in local-only operator state such as `.local/{{COORDINATOR_DIR}}/`, `.local/config/`, or `.local/ssh/`
...
5. **Git operations route through the `committer` subagent (committer-exclusive gate)** — available only where `core/gated-commit` is selected (see `.opencode/docs/git-execution-routing.md` → "Capability condition").
   ...
   - On profiles WITHOUT the capability there is no committer route: do not probe it, never fall back to raw git — preserve the work, report the missing route, and request separately-authorized activation (`capabilities: [core/gated-commit]` in `.vh-agent-harness/vh-harness-profile.yml`) or operator handling.
...
- **All coding modifications, implementation, research, study, and git operations MUST be delegated to the appropriate subagent.** The coordinator remains strictly read-only. The default target is `build`, which owns the full execution context (file reads, edits, test runs, release); git mutations are delegated to the `committer` agent where `core/gated-commit` is selected — load the `gated-commit` skill or see `.opencode/docs/git-execution-routing.md` (on profiles without that capability the committer is not wired: preserve the work, report the missing route, and request separately-authorized activation or operator handling). When the scope is narrow and clearly bounded, direct delegation to a specialist is acceptable: `commit-message` for reviewed commit message drafting, `researcher` for read-only source gathering, `ship-review` for whole-change audits, etc. The coordinator MUST NOT directly edit source code, run git mutations, write implementation files, or accumulate research detail that belongs in a subagent session.
...
- For multi-session coordination work, classify the task as `short`, `medium`, or `long` before fanning out. Use `docs/coordination/TASK_MODES.md` and `docs/coordination/RUNTIME_MODEL.md` to decide whether `.opencode/state/` is enough or whether a local runtime layer under `.local/{{COORDINATOR_DIR}}/` is justified.
...
- Commit gate rule for every agent/session: before any `git commit` attempt, run `commit-reviewer` (typically via `/commit-review`) on the exact slice, read the reviewer response, and stop when it returns blocked/split guidance. (Where `core/gated-commit` is not selected there is no commit route at all — see `.opencode/docs/git-execution-routing.md` → "Capability condition".)
...
`.local/{{COORDINATOR_DIR}}/skill-proposals/` — gitignored LOCAL CANDIDATE
