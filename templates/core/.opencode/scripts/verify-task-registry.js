...
// `--prefix ../../.local/{{COORDINATOR_DIR}}` would resolve isolatedRoot to the
...
function validateIsolatedRoot(prefix, baseRoot) {
    ...
    // .local/{{COORDINATOR_DIR}}, or the candidate leaf itself symlinked out)
...
// .local/{{COORDINATOR_DIR}}/ registry. This MUST stay byte-for-byte consistent
...
function coordinatorRoot() {
    ...
    return override || path.join(repoRoot(), ".local", "{{COORDINATOR_DIR}}");
}
...
function main() {
    ...
    // .local/{{COORDINATOR_DIR}}/tasks/ registry because state-lib's
    ...
    // validation, `--prefix ../../.local/{{COORDINATOR_DIR}}` would resolve
    ...
    try {
        ...
        const firstCloseout = saveCoordinationTaskCloseout(
            ...
            primary.task.task_id,
            {
                ...
                body: [
                    "1. Verified that local task cards persist under `.local/{{COORDINATOR_DIR}}/tasks/`.",
