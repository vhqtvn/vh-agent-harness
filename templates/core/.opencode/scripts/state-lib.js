...
function localCoordinatorRoot() {
    ...
    // when set, the ENTIRE .local/{{COORDINATOR_DIR}}/ subtree (tasks + reports +
    ...
    if (override) return override;
    return path.join(repoRoot(), ".local", "{{COORDINATOR_DIR}}");
}
...
// .local/{{COORDINATOR_DIR}}/skill-proposals/ — transport, not truth. Unlike
