...
- **DEFER routes to the holding area:** DEFER findings are non-blocking. They are NOT transcribed into `docs/planning/backlog.md` directly. The DEFER grammar (trigger.predicate + trigger.params) IS the intake predicate: a DEFER finding is captured into `.local/{{COORDINATOR_DIR}}/tasks/` (via `/write-task`) as a conditional candidate with Notes provenance (`source:review-defer`, the trigger expression, `studied:YYYY-MM-DD`), and reaches the backlog only after the trigger fires + the promoter applies the Definition of Ready. The promoter runs `check-defer-triggers.js` as a review aid. See the `backlog` skill.
