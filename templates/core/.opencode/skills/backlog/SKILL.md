---
name: backlog
description: "Backlog ledger discipline for {{PROJECT_NAME}} — conflict-safe edits to docs/planning/backlog.md (hybrid split-commit) plus DEFER/follow-up curation routing. Load this skill before editing the backlog, when handling a cas_conflict on the backlog, or when deciding where a DEFER/p2 finding should land."
...
> revert it. DEFER/p2 findings land in `.local/{{COORDINATOR_DIR}}/tasks/`
...
- **Format:** the executable spec is `.opencode/scripts/normalize-backlog.js`.
  ...
  batch edit.
- **DEFER / p2 / follow-up:** capture to `.local/{{COORDINATOR_DIR}}/tasks/`
...
### Holding area = `.local/{{COORDINATOR_DIR}}/tasks/`

DEFER findings (from `commit-reviewer`), p2 follow-ups, and other conditional
candidates land in `.local/{{COORDINATOR_DIR}}/tasks/` as **conditional
...
`.local/{{COORDINATOR_DIR}}/tasks/` with provenance, and the promoter decides
