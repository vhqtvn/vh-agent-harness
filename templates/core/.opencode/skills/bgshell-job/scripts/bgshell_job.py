...
def repo_root() -> Path:
    override = os.environ.get("{{PROJECT_SLUG}}_BGSHELL_JOB_REPO_ROOT")
    if override:
...
def opencode_db_path() -> Path | None:
    override = os.environ.get("{{PROJECT_SLUG}}_BGSHELL_OPENCODE_DB")
    if override:
...
def spawn_wrapper(job_dir: Path) -> int:
    ...
    env = os.environ.copy()
    env["{{PROJECT_SLUG}}_BGSHELL_JOB_REPO_ROOT"] = str(repo_root())
