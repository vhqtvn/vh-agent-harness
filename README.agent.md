...
`git --no-pager log -1` or `git -C <repo-root> diff`. shell-guard already
allows these internally, but opencode's `permission.bash` matcher is a
path-blind raw-string glob with no `git -C *` rule, so such forms would prompt
the operator. shell-guard's `tool.execute.before` hook closes that gap with a
**command rewrite** that strips execution-safe global flags *before* the
matcher sees the command, so the existing `git <verb> *` allow rules match the
rewritten form and no prompt fires.

The rewrite is driven by a **flag registry** (`walkGitGlobals` in
...
`--git-dir=<path>`, `--exec-path[=<path>]`, boolean `-p`, etc.):

- **Always-stripped** (display-only, execution-safe to drop): `-p`,
  `--paginate`, `-P`, `--no-pager`. Rewriting `git --no-pager diff x` to
  `git diff x` is a semantic no-op → the allow rule `git diff *` matches → no
  prompt. (The old `--paging=no` synonym was removed — it is not a real git
  flag; the canonical always-strip set is now `-p`/`--paginate`/`-P`/
  `--no-pager`.)
- **Conditionally stripped**: `-C <path>` is stripped only when the path is
  **absolute and identical to the command's working directory** (so
  `git -C <repo-root> log` rewrites to `git log`). An absolute in-project path
  that differs from cwd, or an absolute external path, is **kept** (no rewrite)
  → opencode prompts, which is the safe default.
- **Never stripped** (security/behavior-changing): `-c`, `--git-dir`,
  `--work-tree`, `--namespace`, `--config-env`, `--exec-path`,
  `--no-replace-objects`, `--bare`, and any unknown `-flag`. Their presence
  blocks the rewrite → opencode prompts.
- **Info flags** (`--help`, `--version`, `--html-path`, …) with no verb are
  read-only terminal requests → allowed directly.

Three load-bearing invariants the rewrite preserves:

1. **Mutation-slip guard.** A mutation verb routed past any global flag is
   denied before the allowlist is consulted — `git --no-pager commit`,
...
2. **Relative `-C` is denied with a notice.** Any relative `-C` argument
   (`.`, `..`, a subdir) is rejected: `"relative -C paths are not
   auto-normalized; use an absolute path equal to the working directory or
   drop -C"`. Normalizing relative paths would invite symlink / `..` /
   normalization bugs, so the gate refuses to guess.
3. **External `-C` readonly is `ask`, not silent allow.** `git -C /var/x diff`
   is classified `ask` (the operator sees a prompt); it is never silently
   rewritten to `git diff`. The mutation-slip guard still turns
   `git -C /var/x commit` into a hard `deny`.
