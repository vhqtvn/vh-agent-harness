...
//
// Contents-vs-meta-state distinction (defer-003): the CARD CONTENTS under
// .local/coordinator/tasks/ remain losable transport (gitignored, regenerated by
// coordinator workflows). The coordinator-adoption MARKER at
// .vh-agent-harness/coordinator-adoption.json is channel META-STATE — a tiny
// committed boolean, like the release-defer-dispositions manifest (38c5c477),
// that survives a fresh checkout. Projecting it here does NOT promote transport
// identity to canon; it only lets the gate tell "never adopted" apart from
// "adopted transport lost" so the latter fails closed instead of silently
// skipping. The kernel DERIVES the marker state (absent/valid/corrupt); the gate
// alone maps those states to SKIP/FAIL/PASS — the authority line (kernel
// informs, gate acts).
package claims
...
type Registry struct {
	...
	// gate does with it (the gate maps the state to a tier). When corrupt,
	// AdoptionMarkerDetail carries the reason. This is channel meta-state, not
	// card contents — see the package doc's contents-vs-meta-state distinction.
	AdoptionMarker       AdoptionMarkerState
	AdoptionMarkerDetail string
}

// AdoptionMarkerState classifies the committed coordinator-adoption marker at
// .vh-agent-harness/coordinator-adoption.json (defer-003). The kernel only
// DERIVES this state; it does NOT decide what the gate does with it. The release
// gate (internal/cli/release_gate.go checkDeferLiveness) is the sole transition
// authority that maps these states to tiers — mirroring how validateCardIdentities
// surfaces CardError without deciding the tier. This keeps the authority line
// intact: the kernel informs, the gate acts.
type AdoptionMarkerState string

const (
	// AdoptionMarkerAbsent: no marker file present (greenfield, or a repo that
	// has never run a successful save_coordination_task). The gate treats an
	// absent marker as "not adopted" → SKIP, even if stray task cards exist
	// (transport is not trusted to gate releases until the marker opts in).
	AdoptionMarkerAbsent AdoptionMarkerState = "absent"
	// AdoptionMarkerValid: marker present and parses as the canonical payload
	// {"version":1,"adopted":true}. Combined with TasksPresent the gate runs its
	// normal card/contradiction logic; combined with an absent tasks dir it FAILs
	// (adopted transport lost).
	AdoptionMarkerValid AdoptionMarkerState = "valid"
	// AdoptionMarkerCorrupt: marker present but unreadable, unparseable, or the
	// wrong shape/version. Fail-closed (mirrors CardError discipline).
	AdoptionMarkerCorrupt AdoptionMarkerState = "corrupt"
)
...
func Derive(repoRoot string) (Registry, error) {
	...
	// Channel meta-state: the committed coordinator-adoption marker (defer-003).
	// Distinct from TasksPresent (losable transport): the marker is git-committed
	// meta-state that survives a fresh checkout, so the gate can tell "never
...
// marker at <repoRoot>/.vh-agent-harness/coordinator-adoption.json (defer-003).
// It returns:
//   - AdoptionMarkerAbsent (empty detail) when the file is missing — a clean
//     "never adopted" signal, NOT a failure.
//   - AdoptionMarkerValid (empty detail) when it parses as the canonical
//     payload {"version":1,"adopted":true}.
//   - AdoptionMarkerCorrupt (with a human-readable detail) when it is unreadable,
//     unparseable, or the wrong shape/version — fail-closed like CardError.
//
// The kernel only DERIVES this state; the release gate maps it to a tier. This
// function performs no writes and is side-effect-free (the package doc's
// non-authoritative invariant).
func loadAdoptionMarker(repoRoot string) (AdoptionMarkerState, string) {
	markerPath := filepath.Join(repoRoot, ".vh-agent-harness", "coordinator-adoption.json")
	raw, err := os.ReadFile(markerPath)
	if err != nil {
		if os.IsNotExist(err) {
			return AdoptionMarkerAbsent, ""
		}
		return AdoptionMarkerCorrupt, fmt.Sprintf("read adoption marker %s: %v", markerPath, err)
	}
	var m struct {
		Version int  `json:"version"`
		Adopted bool `json:"adopted"`
	}
	// Decode strictly: DisallowUnknownFields makes a superset payload
	// (e.g. {"version":1,"adopted":true,"extra":...}) fail to parse instead of
	// silently being accepted, so the "exactly the canonical shape" contract
	// below is actually enforced. A present-but-wrong marker is treated as
	// corrupt (fail-closed) rather than silently trusted.
	dec := json.NewDecoder(bytes.NewReader(raw))
	dec.DisallowUnknownFields()
	if err := dec.Decode(&m); err != nil {
		return AdoptionMarkerCorrupt, fmt.Sprintf("parse adoption marker %s: %v", markerPath, err)
	}
	// Require the marker to be EXACTLY one JSON value. Decode reads only the
	// first value, so without this check a payload like
	// {"version":1,"adopted":true}\n{"extra":1} would decode the first object,
	// ignore the trailing second object, and classify as Valid — a fail-open vs
	// the canonical-shape contract. Re-decoding must hit io.EOF (no trailing
	// non-whitespace content).
	if err := dec.Decode(&json.RawMessage{}); err != io.EOF {
		return AdoptionMarkerCorrupt, fmt.Sprintf("parse adoption marker %s: trailing content after JSON value", markerPath)
	}
	// Reject any payload that is not exactly the canonical shape. The producer
	// writes {"version":1,"adopted":true}.
	if m.Version != 1 || !m.Adopted {
		return AdoptionMarkerCorrupt, fmt.Sprintf("adoption marker %s unexpected payload: version=%d adopted=%v (want version=1 adopted=true)", markerPath, m.Version, m.Adopted)
	}
	return AdoptionMarkerValid, ""
}
