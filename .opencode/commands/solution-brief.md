...
- `solution-brief` is read-only on repo source, docs, and durable state; it
  chains `researcher` -> `debate` -> `planner` internally, persists the
  consolidated brief to `tmp/agent-runs/<alias>/brief.md`, and returns that path
  plus a compact summary. The persisted brief is the artifact that survives a
  compaction or `/job-cleanup`; treat the in-message summary as disposable
...
Return:
- the path to the persisted brief under `tmp/agent-runs/<alias>/brief.md`
- decision frame
