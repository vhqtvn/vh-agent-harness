...
EOF
  ...
  # the diagnostic instead of discarding it. EEXIST ("File exists") is the ONE
  # failure that means a genuine race — keep the historical contended/
  # race_lost semantics for it. Every other errno (ENOTDIR: a path component
  # is a file — the linked-worktree failure mode where `.git` is a file;
  # EACCES: unwritable git dir; ENOENT: missing parent) means the LOCK DIR
  # ITSELF IS UNUSABLE — NOT contention — and MUST surface as a distinct
  # error status so a retry loop cannot mistake an environment failure for a
  # losable race (the shipped bug: stderr was suppressed and ENOTDIR was
  # masked as contended/race_lost with an empty holder, inviting infinite
  # retries). LC_ALL=C pins the mkdir diagnostic to English so the EEXIST
  ...
  if ! mkdir_err=$(LC_ALL=C mkdir "$LOCK_DIR" 2>&1); then
    ...
    # distinct error, never as contention.
    local lock_err_enc
  ...
  # prep-tempdir resolves the same per-invocation git dir (see
  # readonly-scripts.sh), so it creates the same directory as GATE_INDEX_DIR.
  ...
  # write_tree_failed errors). We already hold the lock at this point but
  # have NOT yet written LOCK_META — remove the half-born lock so the gate is
  # not left held (a meta-less lock is unbreakable by _stale_break's
  # fail-safe).
