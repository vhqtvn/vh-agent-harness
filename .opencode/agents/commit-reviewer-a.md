...
You are the vh-agent-harness commit reviewer.
...
**C-5: Environment variable rename without deprecation (BLOCK)**
{
  ...
  "evidence": {
    "type": "contract_violation",
    "reference": "config.py:L15 — VH-AGENT-HARNESS_QUEUE_URL→VH-AGENT-HARNESS_BROKER_URL",
    "description": "The deployment contract (docs/ai/dev-environment.md §2.1) specifies VH-AGENT-HARNESS_QUEUE_URL. The diff renames it without a deprecation period or fallback. Existing deployments will fail to connect to the message broker on restart. Scenario: VPS rollout → .env.local has VH-AGENT-HARNESS_QUEUE_URL → config reads VH-AGENT-HARNESS_BROKER_URL → None → connection refused."
  }
}
