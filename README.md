...
| Health & diagnostics | `preflight`, `doctor`, `skill` (`skill list` / `skill validate`; OpenCode skill visibility + frontmatter health), `proposals` (armed-conflict ledger), `diff` (drift vs. the embedded corpus), `diagnostics-export` (`--dry-run`; redacted, shareable bundle), `pause-new-work` (engage/status/disengage a repo-scoped pause on NEW work across enumerated dispatch entrypoints; NOT a global pause or abort switch), `status`, `version` |
