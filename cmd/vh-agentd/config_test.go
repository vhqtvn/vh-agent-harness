...
func TestValidateMaxTokensAndTimeoutBounds(t *testing.T) {
	...
	if _, err := validate("openai", "m", "http://x.test", "K", "/tmp/vh-agentd-test-sessions", "", -1, defaultApprovalTimeoutMs, 0, "off"); err == nil || !strings.Contains(err.Error(), "--max-tokens") || !strings.Contains(err.Error(), "must be >= 0") {
...
func TestValidateOptimizerFlag(t *testing.T) {
	for in, want := range map[string]string{"": "llm", "llm": "llm", "LLM": "llm", " llm ": "llm", "dedup": "dedup", "Dedup": "dedup"} {
		cfg, err := validate("openai", "m", "http://x.test", "K", "/tmp/vh-agentd-test-sessions", in, 0, defaultApprovalTimeoutMs, 0, "off")
	...
	for _, bad := range []string{"bogus", "openai", "dedupe"} {
		_, err := validate("openai", "m", "http://x.test", "K", "/tmp/vh-agentd-test-sessions", bad, 0, defaultApprovalTimeoutMs, 0, "off")
