...
// list, surfaced out-of-the-box by name. The `repo-mail` pack is the third
// shipped embedded pack: the repo-mail inter-repo communication protocol
// overlay (egress-gate wiring), also overlay-only and selected via `overlays:`.
...
// (KnownPacks/OpenPack/Pack) lists any directory dropped here automatically —
// no code change. The directory retains a .gitkeep so the embed directive still
