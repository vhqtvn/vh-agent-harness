...
#
# Usage (manifest-authority release with explicit operator override ceremony):
...
#   $RELEASE_DEFER_MANIFEST_AUTHORITY -> optional; "1" or "true" activates
#                                  manifest-authority release mode (reads the
#                                  committed manifest, no .local/ access). When
#                                  unset the wrapper stays on legacy release
#                                  mode for backward compatibility. This env is
#                                  the phased-activation switch.
#
...
# `disclosures` and `accepted_overrides` are ALWAYS present (additive over the
# original 5-field legacy shape). In legacy release mode they are empty arrays
# `[]`; in manifest-authority mode they carry the evaluator's disclosures and
# any operator-approved overrides.
...
#
# In manifest-authority mode, disclosures and accepted overrides are also
# printed to stderr BEFORE the `git tag -a` mutation so the operator can
# observe exactly what will be released. The same data is embedded in the
# final JSON for machine consumers.
set -euo pipefail
...
#
# These flags are the OPERATOR-SIDE override ceremony. They have no effect in
# legacy release mode. In manifest-authority mode they are the only way an
...
# --- manifest-authority activation (phased-activation switch) ---
#
# When RELEASE_DEFER_MANIFEST_AUTHORITY is 1 or true, the wrapper:
#   1. Sets the same env when invoking the evaluator → evaluator runs
#      mainReleaseManifest (reads committed manifest, NO .local/ access).
#   2. Passes --release-version $VERSION so override binding can compare.
#   3. Performs the operator-side override ceremony against the actual
#      committed manifest blob SHA and forwards --override-confirmed-version
#      ONLY when the ceremony succeeds.
# When unset (or any other value), the wrapper stays on legacy release mode
# so the repo is not broken between Phase 1 (parser landed) and Phase 3
# (operator seeds the manifest). The evaluator's legacy behavior is
# byte-identical to pre-Phase-2; the WRAPPER output is additive — legacy
# mode still emits `disclosures` and `accepted_overrides` fields, but as
# empty arrays `[]` (the legacy evaluator produces no disclosures).
MANIFEST_AUTHORITY="${RELEASE_DEFER_MANIFEST_AUTHORITY-}"
case "$MANIFEST_AUTHORITY" in
  1|true) MANIFEST_AUTHORITY_ACTIVE=1 ;;
  *) MANIFEST_AUTHORITY_ACTIVE=0 ;;
esac

# --- override ceremony (manifest-authority mode only) ---

CONFIRMED_VERSION=""
if [ "$MANIFEST_AUTHORITY_ACTIVE" = "1" ]; then
...
#   MANIFEST-AUTHORITY mode (env=1|true):
#     committed manifest missing  → REFUSE (evaluator-error)
#     manifest malformed / handshake mismatch → REFUSE (evaluator-error)
#     blocker / evaluator-error   → REFUSE before mutation
...
DEFER_EXIT=0
if [ "$MANIFEST_AUTHORITY_ACTIVE" = "1" ]; then
  DEFER_OUTPUT=$(RELEASE_DEFER_MANIFEST_AUTHORITY=1 node .opencode/scripts/check-defer-triggers.js "${DEFER_ARGS[@]}" 2>/dev/null) || DEFER_EXIT=$?
else
...
# --- operator-ceremony enforcement (manifest-authority mode only) ---
#
...
if [ "$MANIFEST_AUTHORITY_ACTIVE" = "1" ] && [ "$PARSED_OK" = "1" ]; then
...
# --- pre-tag disclosure print (manifest-authority mode only) ---
#
# In manifest-authority mode the operator sees disclosures + accepted
# overrides on stderr BEFORE the `git tag -a` mutation. The same data is
# embedded in the final JSON. Legacy mode typically has empty arrays here.
if [ "$MANIFEST_AUTHORITY_ACTIVE" = "1" ] && [ "$PARSED_OK" = "1" ]; then
