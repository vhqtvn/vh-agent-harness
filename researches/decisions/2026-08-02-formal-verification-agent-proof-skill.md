...
> Pilot #1 (exec-sandbox mode-floor invariant): engine-checked green offline (Lean 4.32.2, `formal-verify/lean4:v4.32.2`, `--network=none`, exit 0); red-on-divergence demonstrated — observed: flipping max→min in `internal/execsandbox/floor.go::ApplyFloor` (changing the condition `modeRank(floor) > modeRank(requested)` from `>` to `<`, i.e. returning the floor when it is the *min*) and running `TestApplyFloorToRequest_FloorSafetyInvariant_FVBinding` in `internal/cli/exec_sandbox_floor_test.go` — the 3×3 = 9-cell `(requested, floor)` matrix the pilot added (committed test `4caa3c8`) — produced **6/9 cells RED** (`go test -v` output captured): the 3 `floor=best-effort` cells and the 3 `floor=strict` cells went RED; the 3 `floor=off` cells stayed GREEN. The flip was reverted and the suite re-confirmed GREEN at HEAD. (The 6 — not the 3 an isolated trace of `ApplyFloor` alone yields — arises because `applyFloorToRequest` calls `ApplyFloor` *twice*: once to aggregate the floor (`floor = ApplyFloor(ModeOff, loaded)`) and once to apply it; under min, the aggregation collapses the floor to `ModeOff`, so `effective = off` for every cell, and the `rank(effective) ≥ rank(floor)` invariant fails for all 6 cells whose fixture floor is above `off`. This is why a prior memo claim of 3/9 was an unrun tracing artifact; the observed count is 6/9. A second, separate floor test — `TestApplyFloorToRequest`, the 10-case end-to-end clamp test — went 9/10 RED under the same flip, but the 9-cell figure is the one the FV-pilot binding targets.) fidelity binding written. The test going RED on divergence is the crux and holds either way; the count is stated as observed with method, not traced. First plumbing validation — one datapoint toward the S2 two-sided health signal (catches-real-divergence side). NOT yet S2-sufficient (one pilot, one-sided test).
