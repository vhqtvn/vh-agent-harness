...
- `README.agent.md` — two additions to the repo-root hand-maintained file (not a generated
  mirror): (1) the read-only verb inventory section gains the family-RO rule + fail-open caveat;
  (2) a new "Capability residue (deselected capability; NOT an orphan)" section documents that
  residue is a known path exempt from drift and doctor, that `--prune-orphans` does NOT target it,
  and that the supported no-orphan cleanup is whole-directory removal of `.opencode/skills/<name>/`
  (plus the capability-owned agent file, e.g. `.opencode/agents/media-perception.md`).
- `templates/migrations/v0.15.0.md` — **NOT edited in this slice.** The same residue clarification
  was drafted here first, but `templates/migrations/v0.15.0.md` is a *released* note. The
  immutability gate `TestMigrationNotes_ReleasedImmutable` enforces that corrections to a released
  note ship as an erratum in the *next* release's note, never via a working-tree edit. The draft
  edit was reverted to HEAD byte-identical and the wording was relocated to `README.agent.md`
  (above) and `permission-templates.md` instead. A future release note may carry the residue
  guidance as erratum wording if it still reads as a release-relevant correction then.
...
| docs lands this slice | `internal/permconfig/tables.go` (comment-only), `templates/core/.opencode/docs/agents/permission-templates.md` (3a), `README.agent.md` (RO inventory + new "Capability residue" section) | yes |
| v0.15.0.md edit reverted (released-note immutability) | `TestMigrationNotes_ReleasedImmutable` (`internal/cli/migration_release_test.go:129`) blocks working-tree edits to released notes; `git diff --stat templates/migrations/v0.15.0.md` empty | yes |
