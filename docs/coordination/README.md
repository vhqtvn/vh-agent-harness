...
| Live task-card transport | `.local/coordinator/tasks/` | Gitignored transport for worker status intents (`/write-task`, `/task-update`, `/task-closeout`, `/task-review`). NOT truth — the promoter promotes consolidated results into `backlog.md` per cycle. |
...
(`.local/coordinator/tasks/`) via the lifecycle commands; the promoter
...
1. Keep `docs/planning/backlog.md` as the only committed task-status source of
   ...
   `.local/coordinator/tasks/` (transport), never direct backlog edits.
